...
#define LAC_X 716
#define LAC_Y 512
#define LAC_WIDTH 128
#define LAC_HEIGHT 128
...
int choisir_action_softmax(float *tab_interet, int nb_actions) {
  ...
  for (int i = 1; i < nb_actions;
       i++) { // ici on l'action avec le meilleur interet
...
Goat *init_goat(Goat *g) {
  do {
    g->x = MARGE + rand() % (LARGEUR - 2 * MARGE - WIDTH_GOAT);
    g->y = MARGE + rand() % (HAUTEUR - 2 * MARGE - HEIGHT_GOAT);
  } while (check_collision_rect(g->x, g->y, WIDTH_GOAT, HEIGHT_GOAT, LAC_X,
                                LAC_Y, LAC_WIDTH, LAC_HEIGHT));
...
Wolf *init_wolf(Wolf *l) {
  do {
    l->x = MARGE + rand() % (LARGEUR - 2 * MARGE - WIDTH_WOLF);
    l->y = MARGE + rand() % (HAUTEUR - 2 * MARGE - HEIGHT_WOLF);
  } while (check_collision_rect(l->x, l->y, WIDTH_WOLF, HEIGHT_WOLF, LAC_X,
                                LAC_Y, LAC_WIDTH, LAC_HEIGHT));
...
/* Entrée : le monde actuel
   ...
   notre jeu */
monde *mis_à_jour_monde(monde *monde_courant, int tick_animation, int input_x,
                        int input_y) {
  // =========
  // Décision
  // =========
  ...
  action_fermier =
      decider_action_fermier(monde_courant->fermiers, perception_fermier);
  ...
  for (int i = 0; i < monde_courant->nb_goat; i++) {
    ...
    current_goat->action_choisi =
        decider_action_goat(current_goat, perception_goat);
  ...
  for (int i = 0; i < monde_courant->nb_wolf; i++) {
    ...
    current_wolf->action_choisi =
        decider_action_wolf(current_wolf, perception_wolf);
  ...
  // Normalisation de la vitesse en diagonale pour conserver une vitesse
  // constante
  ...
  for (int i = 0; i < monde_courant->nb_goat; i++) {
    ...
    // Collision avec le lac
    if (check_collision_rect(next_x, next_y, WIDTH_GOAT, HEIGHT_GOAT, LAC_X,
                             LAC_Y, LAC_WIDTH, LAC_HEIGHT)) {
      collision = 1;
  ...
  for (int i = 0; i < monde_courant->nb_wolf; i++) {
    ...
    // Collision avec le lac
    if (check_collision_rect(next_x, next_y, WIDTH_WOLF, HEIGHT_WOLF, LAC_X,
                             LAC_Y, LAC_WIDTH, LAC_HEIGHT)) {
      collision = 1;
    ...
    for (int j = 0; j < monde_courant->nb_wolf; j++) {
      if (i != j) {
        if (check_collision_rect(next_x, next_y, WIDTH_WOLF, HEIGHT_WOLF,
                                 monde_courant->wolfs_tab[j]->x,
                                 monde_courant->wolfs_tab[j]->y, WIDTH_WOLF,
                                 HEIGHT_WOLF)) {
          collision = 1;
          current_wolf->timer_mouvement = 0;
          break;
